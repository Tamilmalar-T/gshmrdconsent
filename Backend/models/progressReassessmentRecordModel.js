...
class ProgressReassessmentRecordModel {
  static async create(patientId, formData) {
    const query = `
      INSERT INTO progressreassessmentrecord (patient_id, form_data)
      VALUES ($1, $2)
      RETURNING *;
    `;
  ...
  static async getByPatientId(patientId) {
    const query = 'SELECT * FROM progressreassessmentrecord WHERE patient_id = $1 ORDER BY created_at DESC;';
